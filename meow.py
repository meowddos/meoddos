...
def check_socks():
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("Đã kiểm tra "+str(nums)+" cặc\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("Đã kiểm tra "+str(nums)+" viên đạn\r")
		sys.stdout.flush()
	print("\r\nKiểm tra cặc xong, Số Cặc Bự:"+str(len(proxies)))
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]Đang xả vào mặt "+ip+":"+str(port)+" code by meow\r")
            sys.stdout.flush()
            n +=1
